...
export default function AssetCard({ asset, projectId, onUpdate, onDelete }: AssetCardProps) {
  ...
  return (
    <>
      <div
        className="bg-gray-800 border border-gray-700 rounded-lg overflow-hidden cursor-pointer hover:border-gray-500 transition-colors"
        onClick={() => setExpanded(true)}
      >
        {/* 缩略图 */}
        <div className="h-20 bg-gray-900 flex items-center justify-center relative">
          {loadingImages ? (
            <span className="text-xs text-gray-600">...</span>
          ) : primaryImageUrl ? (
            <img src={primaryImageUrl} alt={asset.name} className="w-full h-full object-cover" />
          ) : (
        ...
        <div className="px-2 py-1.5">
          <div className="flex items-center gap-1 mb-0.5">
            <span className={`text-xs px-1.5 py-0.5 rounded ${TYPE_COLOR[asset.asset_type]}`}>
              {TYPE_LABEL[asset.asset_type]}
            </span>
          </div>
          <p className="text-sm font-medium text-white truncate">{asset.name}</p>
          <p className="text-xs text-gray-500 truncate">
            {asset.appearance || asset.description || asset.asset_id}
          </p>
        </div>
      </div>
      ...
      {expanded && (
        ...
        >
          ...
          >
            {/* 图片区 */}
            <div className="h-56 bg-gray-950 flex items-center justify-center relative">
              {generating ? (
                ...
                <>
                  ...
                  {asset.asset_type === 'character' && (
                    <div className="space-y-1 text-sm text-gray-400">
                      {asset.gender && (
                        <p><span className="text-gray-500">性别：</span>{asset.gender}{asset.age && ` · ${asset.age}`}</p>
                      )}
                  ...
                  {asset.asset_type === 'scene' && (
                    <div className="space-y-1 text-sm text-gray-400">
                      ...
                      {asset.time_of_day && (
                        <p><span className="text-gray-500">时间：</span>{asset.time_of_day}{asset.lighting && ` · ${asset.lighting}`}</p>
                      )}
                    </div>
                  )}
