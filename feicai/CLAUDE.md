...
## 资产提取架构（v1.7 确认，2026-04）

**只有 Path B**，Path A 已废弃：

| 路径 | 说明 | 状态 |
|------|------|------|
| Path A 逐集坍缩 | 单集 asset_refs → 无跨集去重 | **已废弃，前端按钮已下线** |
| Path B 两阶段批量 | Phase1 提取原始 mentions → 程序聚类 → Phase2 LLM 提炼 | **唯一出口** |

Path B 提取完成后写入 `episode_assets.json`（每集），记录本集使用的 asset_id 索引。提示词生成阶段读该文件按集过滤资产注入。

**场景聚类规则**：`_name_similar()` 对场景类型使用 Jaccard bigram ≥ 50% 阈值（角色/道具保持宽松规则）。
...
- ✅ Tab1 资产库：CRUD + 图片 + 变体 + Path B 全集批量提取
- ✅ Tab2 分镜规划：AI 规划 + 组视图 + 编辑面板（枚举容错，失败透出 warnings）
- ✅ Tab3 装配：组导航 + 镜头列表 + 参数面板 + 提示词导出
- ✅ Tab4 质检：矩阵视图 + 时长统计 + 视频预览 + 剪映导出
- ✅ 设置页：LLM / 预设 / 系统提示词 / 即梦 / 提供商 / 视频参数
- ✅ 装扮注册表（costume_registry.json）项目级积累
- ✅ 提示词生成按集过滤资产（episode_assets.json 索引）
- 🚧 视频生成对接即梦 CLI（待用户提供命令格式）

---

## 第二批待做功能（已记录，未开发）

1. **分镜规划注入前情提要**：读上一集 summary.txt 注入 LLM 上下文，增强跨集剧情连贯性
2. **并发分镜规划**：用户可配置并发数（1/2/3），需先修复 costume_registry 写入竞争问题
3. **LLM 全局限速**：asyncio.Semaphore 上限 3，防止并发过高触发 API 限速
4. **任务监控面板改造**：更丰富信息（进度/耗时/token用量）+ 程序层取消按钮
5. **批量规划一键入口**：Tab2 增加「批量规划所有集」按钮，顺序链式执行
6. **提示词生成规则迭代**：当前规则较基础，后续持续优化 system prompt
7. **分镜规划注册表按集过滤**：避免 EP80 规划时注入无关角色的装扮词
8. **Phase 2 fragments 程序去重**：主角 80 集重复描述优化，减少 LLM 上下文
